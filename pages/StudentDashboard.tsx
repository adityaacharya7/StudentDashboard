    ...
    <div className="p-6 md:p-10 max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500">
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="flex items-center gap-2 px-4 py-2 bg-white dark:bg-slate-900 border dark:border-slate-800 rounded-full shadow-sm">
          ...
          <span className="text-sm font-bold text-gray-600 dark:text-gray-300">Synapse AI</span>
        </div>
      </header>
        ...
        {quickLinks.map(item => (
          <Link key={item.path} to={item.path}
            ...
            <div className="relative z-10">
              ...
              <div className="mt-4 flex items-center gap-1 text-white/60 text-xs font-bold group-hover:text-white/90 group-hover:gap-2 transition-all">
            ...
            {last7.map((d, i) => (
              <div key={i} className="flex flex-col items-center gap-1.5">
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center transition-all ${
                  d.active ? 'bg-orange-500 text-white shadow-lg shadow-orange-500/20' : 'bg-gray-100 dark:bg-slate-800 text-gray-300 dark:text-gray-600'
                }`}>
        ...
        <div className="bg-white dark:bg-slate-950 border border-gray-100 dark:border-slate-800 rounded-3xl p-6 shadow-sm">
          ...
          {showAddExam && (
            <div className="flex gap-2 mb-3">
              <input value={newExamSubject} onChange={e => setNewExamSubject(e.target.value)} placeholder="Subject" className="flex-1 p-2 bg-gray-50 dark:bg-slate-800 rounded-lg text-xs font-bold dark:text-white outline-none" />
              <input type="date" value={newExamDate} onChange={e => setNewExamDate(e.target.value)} className="p-2 bg-gray-50 dark:bg-slate-800 rounded-lg text-xs font-bold dark:text-white outline-none" />
          ...
          {exams.length === 0 ? (
            ...
            <div className="space-y-2 max-h-28 overflow-y-auto">
              {exams.map(ex => {
                ...
                return (
                  <div key={ex.id} className="flex items-center justify-between p-2.5 bg-gray-50 dark:bg-slate-800 rounded-xl group">
                    <div className="flex items-center gap-3">
                      <span className={`text-[10px] font-black px-2 py-0.5 rounded ${urgent ? 'bg-red-100 text-red-600 dark:bg-red-900/30 dark:text-red-400' : 'bg-gray-100 dark:bg-slate-700 text-gray-500'}`}>{daysText}</span>
                      <span className="text-sm font-bold dark:text-white">{ex.subject}</span>
                    </div>
                    <button onClick={() => removeExam(ex.id)} className="p-1 text-gray-300 opacity-0 group-hover:opacity-100 hover:text-red-500 transition-all"><Trash2 size={12} /></button>
                  </div>
                );
              })}
            </div>
          ...
          <div className="bg-white dark:bg-slate-950 border border-gray-100 dark:border-slate-800 rounded-3xl p-8 shadow-sm">
            ...
            {loading ? (
              <div className="animate-pulse space-y-3">
                {[1, 2, 3].map(i => <div key={i} className="h-14 bg-gray-100 dark:bg-slate-900 rounded-2xl"></div>)}
              </div>
              ...
              <div className="text-center py-10">
                ...
                <p className="text-gray-400 text-sm mt-1">Take a quiz, summarize notes, or analyze your scores to get started!</p>
              </div>
            ) : (
              <div className="space-y-3">
                {recentActivity.map((item, i) => {
                  ...
                  return (
                    <div key={i} className="flex items-center gap-4 p-4 bg-gray-50 dark:bg-slate-900/50 rounded-2xl hover:bg-gray-100 dark:hover:bg-slate-800 transition-colors">
          ...
          {latestReport && (
            <div className="bg-white dark:bg-slate-950 border border-gray-100 dark:border-slate-800 rounded-3xl p-8 shadow-sm animate-in fade-in duration-500">
              <div className="flex justify-between items-center mb-6">
                ...
                <Link to="/performance" className="text-xs font-bold text-brand-600 hover:underline flex items-center gap-1">
                  View All <ChevronRight size={12} />
                </Link>
              </div>
              <div className="flex items-center gap-6 mb-6">
                <div className={`text-4xl font-black ${latestReport.overallPercent >= 70 ? 'text-green-500' : latestReport.overallPercent >= 50 ? 'text-yellow-500' : 'text-red-500'}`}>
                  {latestReport.overallPercent}%
                </div>
                <div>
                  <p className="font-bold dark:text-white">{latestReport.examName}</p>
                  <p className="text-xs text-gray-400">{latestReport.subjects.length} subjects • {new Date(latestReport.createdAt).toLocaleDateString()}</p>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div className="p-4 bg-green-50 dark:bg-green-900/10 rounded-2xl">
                  <p className="text-[10px] font-black text-green-600 uppercase tracking-widest mb-2">Strongest</p>
                  <p className="text-sm font-bold text-green-800 dark:text-green-300">{latestReport.strengths[0] || '—'}</p>
                </div>
                <div className="p-4 bg-red-50 dark:bg-red-900/10 rounded-2xl">
                  <p className="text-[10px] font-black text-red-600 uppercase tracking-widest mb-2">Focus Area</p>
          ...
          <div className="bg-brand-50 dark:bg-brand-950/30 border border-brand-100 dark:border-brand-900/50 p-6 rounded-3xl relative overflow-hidden group">
            ...
            <div className="relative z-10">
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="text-xs font-black text-brand-600 dark:text-brand-300 uppercase tracking-widest">Daily Study Tip</span>
              </div>
              <p className="text-gray-800 dark:text-gray-200 font-bold text-lg leading-relaxed">
                "{dailyTip}"
              </p>
              <div className="mt-6 pt-4 border-t border-brand-200 dark:border-brand-900/50">
                <Link to="/advisor" className="text-sm font-black text-brand-600 dark:text-brand-400 flex items-center gap-2 hover:gap-3 transition-all">
...
const QuickToolLink = ({ label, desc, icon, path }: { label: string; desc: string; icon: React.ReactNode; path: string }) => (
  <Link to={path} className="flex items-center gap-3 p-3 rounded-2xl hover:bg-gray-50 dark:hover:bg-slate-900 transition-colors group">
    <div className="w-10 h-10 bg-gray-100 dark:bg-slate-800 rounded-xl flex items-center justify-center text-gray-500 dark:text-gray-400 group-hover:text-brand-600 transition-colors">
      {icon}
    </div>
